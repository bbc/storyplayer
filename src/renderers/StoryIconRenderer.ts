...
export default class StoryIconRenderer extends EventEmitter {
    ...
    >
    _player: Player
    _preloadedIcons: Array<HTMLImageElement>
    ...
    start(startingRepresentationId: string) {
        if (this._pathItemList.length <= 0) {
            return
        }
        ...
        this._getIconAssets().then(iconAssets => {
            this._iconUrlMap = StoryIconRenderer._buildUrlMap(
                this._pathItemList,
                iconAssets,
            )
            this.preloadIcons()
    ...
    _handleIconClicked(event: Record<string, any>) {
        const pathItem = this._pathItemList.find(
            i => i.representation && i.representation.id === event.id,
        )

        if (pathItem) {
            this.emit(
                REASONER_EVENTS.JUMP_TO_NARRATIVE_ELEMENT,
                pathItem.narrative_element.id,
            )
        }
    }
    ...
    handleNarrativeElementChanged(representationId: string) {
        ...
        Object.keys(this._iconUrlMap).forEach(mapKey => {
            ...
            const className = `chapter${this._getRepresentationIndex(
                representationId,
            )}`
    ...
    _findSubStories(): string {
        ...
        Object.keys(this._iconUrlMap).forEach((representationId, index) => {
            if (this._iconUrlMap[representationId] !== null) {
                // there is an icon
        ...
        const commonPath = StoryIconRenderer._findLongestCommonList(
            activeElements,
        )
