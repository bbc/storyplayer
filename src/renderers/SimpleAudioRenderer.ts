...
export default class SimpleAudioRenderer extends BaseTimedMediaRenderer {
    ...
    async _renderBackgroundImage() {
        ...
        logger.info(
            `Rendering background image for audio representation ${this._representation.id}`,
        )

        if (this._representation.asset_collections.background_image) {
            try {
                const assetCollectionId = this._representation.asset_collections
                    .background_image
                const image = await this._fetchAssetCollection(
                    assetCollectionId,
                )
                ...
                if (image.assets.image_src) {
                    ...
                    this._backgroundImage.className =
                        "romper-render-image notInteractiveContent noselect"
    ...
    _setImageVisibility(visible: boolean) {
        if (this._backgroundImage)
            this._backgroundImage.style.opacity = visible ? "1" : "0"
    }
    ...
    destroy() {
        ...
        if (this._backgroundImage)
            this._target.removeChild(this._backgroundImage)
        return true
    }
}
