...
export default class SimpleTextRenderer extends BaseTimedIntervalRenderer {
     ...
     */
        ...
        try {
            ...
            if (guiLayer) guiLayer.classList.remove("overflowing-text")
        } catch (e) {
    ...
    renderTextElement() {
        ...
        if (this._representation.asset_collections.foreground_id) {
            return this._fetchAssetCollection(
                ...
                return Promise.reject(
                    new Error("No text_src in foreground asset collection"),
                )
            })
        }
        ...
        if (this._representation.description) {
            ...
            logger.warn(
                "Text Renderer has no asset collection - rendering description",
            )
    ...
    _fetchTextContent(mediaUrl: string) {
        fetch(mediaUrl)
            .then(response => {
                ...
                return Promise.reject(
                    new Error(`Invalid response: ${response.toString()}`),
                )
            })
            ...
            .catch(rejection => {
                ...
                logger.error(
                    `could not fetch text content ${mediaUrl}: ${rejection.status} ${rejection.statusText}`,
                )
            })
    }
    ...
    populateTextElement(textContent: string) {
        replaceEscapedVariables(textContent, this._controller).then(newText => {
