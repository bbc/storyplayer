...
export default class SwitchableRenderer extends BaseRenderer {
    ...
    async _initChosenRenderer() {
        ...
        if (this._choiceRenderers.length > this._currentRendererIndex) {
            const firstChoice = this._choiceRenderers[
                this._currentRendererIndex
            ]
    ...
    _preloadSwitchIcons() {
        ...
        if (this._representation.choices) {
            this._representation.choices.forEach(async choice => {
                if (
                    choice.choice_representation &&
                    ...
                    try {
                        ...
                        logger.error(
                            err,
                            `could not preload switchable behaviour asset ${acid}`,
                        )
                    }
                }
            })
    ...
    async _renderSwitchButtons() {
        ...
        repIconObjects.forEach(repObj => {
            ...
            this._player.addRepresentationControl(`${idx}`, iconUrl, choiceName)
        })
    ...
    switchToRepresentationAtIndex(choiceIndex: number) {
        ...
        if (
            ...
            if (
                choiceIndex >= 0 &&
                choiceIndex < this._choiceRenderers.length
            ) {
        ...
        if (choiceIndex >= 0 && choiceIndex < this._choiceRenderers.length) {
            const currentChoice = this._choiceRenderers[
                this._currentRendererIndex
            ]
            ...
            if (currentChoice) {
                ...
                if (currentTimeData.timeBased) {
                    // store playhead time
                    this._previousRendererPlayheadTime =
    ...
    _logSwitch() {
        ...
        if (this._choiceRenderers[this._currentRendererIndex]) {
            targetName = this._choiceRenderers[this._currentRendererIndex]
    ...
    _handleChoiceClicked(event: Record<string, any>): void {
    ...
    getRepresentation() {
        if (
            this._representation.choices &&
            this._representation.choices.length >= this._currentRendererIndex
        ) {
            const choice = this._representation.choices[
                this._currentRendererIndex
            ]
