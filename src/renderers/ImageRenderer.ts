...
export default class ImageRenderer extends BaseTimedIntervalRenderer {
    ...
    start() {
        ...
        if (duration === Infinity) {
            logger.info(
                `Image representation ${this._representation.id} persistent`,
            )
            ...
            logger.warn(
                `Image representation ${this._representation.id} has zero duration`,
            )
            this.complete()
        } else {
            // eslint-disable-next-line max-len
            logger.info(
                `Image representation ${this._representation.id} timed for ${duration}s, starting now`,
            )
            ...
            this.addTimeEventListener(
                ...
                () => {
                    // eslint-disable-next-line max-len
                    logger.info(
                        `Image representation ${this._representation.id} completed time`,
                    )
    ...
    _setVisibility(visible: boolean) {
        if (this._imageElement)
            this._imageElement.style.opacity = visible ? "1" : "0"
    }
