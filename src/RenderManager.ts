...
export default class RenderManager extends EventEmitter {
    ...
    _backgroundRenderers: Record<string, BackgroundRenderer>
    _target: HTMLElement
    ...
    _activeRenderers: Record<string, BaseRenderer>
    _upcomingBackgroundRenderers: Record<string, BackgroundRenderer>
    _previousNeId: string | null | undefined
    ...
    constructor(
        ...
        this._player.on(PlayerEvents.NEXT_BUTTON_CLICKED, () => {
            if (this._currentRenderer) {
                ...
                if (duration === Infinity) {
                    logger.info(
                        "Next button clicked during infinite representation - completing element",
                    ) // eslint-disable-line max-len

                    this._currentRenderer.complete()
                } else if (choiceTime > 0 && currentTime < choiceTime) {
                    logger.info(
                        "Next button clicked on element with choices, skip to them",
                    )
                    rend.setCurrentTime(choiceTime)
                } else if (
                    rend.hasVariablePanelBehaviour() ||
                    ...
                    logger.info(
                        "Next button ignored due to variable panel/choices, skip to end",
                    )

                    // skip to end if we have time-based media
                    // (if not, will continue to play then trigger another ended event)
                    const playoutDuration = this._player.playoutEngine.getDuration(
                        representationId,
                    )
        ...
        this._player.on(DOM_EVENTS.TOGGLE_FULLSCREEN, event =>
    ...
    _handleVisibilityChange(isVisible: boolean) {
        ...
        if (!isVisible) {
            ...
            if (
                ...
                !this._currentRenderer.hasMediaEnded()
            ) {
            ...
            if (
                this._player.playoutEngine.hasStarted() &&
                this._player.userInteractionStarted()
            ) {
                this._player.playoutEngine.playBackgrounds()
            }

            if (this._player._choiceCountdownInterval && this._currentRenderer) {
                // restart countdown
     ...
     */
    async fetchStartImage(story: Record<string, any>) {
        let onLaunchConfig = {
        ...
        if (story.meta && story.meta.romper && story.meta.romper.onLaunch) {
            onLaunchConfig = Object.assign(
                onLaunchConfig,
                story.meta.romper.onLaunch,
            )

            try {
                const fg = await this._fetchers.assetCollectionFetcher(
                    onLaunchConfig.background_art_asset_collection_id,
                )
                ...
                if (fg.assets.image_src) {
                    ...
                    return this._player.setupExperienceOverlays(
                        Object.assign(onLaunchConfig, {
                ...
                logger.error(
                    err,
                    `Could not resolve splash image asset: ${err}`,
                )

                this._player.setupExperienceOverlays(
                    Object.assign(onLaunchConfig, {
                        background_art: this._assetUrls.noBackgroundAssetUrl,
                    }),
                )
    ...
    _setAspectRatio(story: Record<string, any>) {
    ...
    handleNEChange(narrativeElement: NarrativeElement) {
                ...
                .then(representation => {
                    if (
                        ...
                        this._currentNarrativeElement.id ===
                            narrativeElement.id &&
                        this._currentRenderer && // need to use _representation as switchable getRepresentation
                        // reports current choice id
                        this._currentRenderer._representation.id ===
                            representation.id
                    ) {
                        this._restartCurrentRenderer()
                    } else {
                        // get a Renderer for this new NE
                        const newRenderer = this._getRenderer(
                            narrativeElement,
                            representation,
                        )
                        ...
                        if (newRenderer) {
                            ...
                            this._handleBackgroundRendering(
                                newRenderer.getRepresentation(),
                            )
                        }

                        // tell story renderer that we've changed
                        if (this._renderStory) {
                            this._renderStory.handleNarrativeElementChanged(
                                representation.id,
                            )
                        }
                    }
        ...
        return Promise.reject(
            new Error("No representation_collection_target_id on NE"),
        )
    }
    ...
    _createStoryIconRenderer(storyItemPath: Array<StoryPathItem>) {
        ...
        this._renderStory.on(
            REASONER_EVENTS.JUMP_TO_NARRATIVE_ELEMENT,
            neid => {
        ...
        if (this._currentRenderer) {
            this._renderStory.start(
                this._currentRenderer.getRepresentation().id,
            )
        }
    }
    ...
    stopCurrentBackgroundRenderers(
        assetCollections: AssetCollection[] | null | undefined,
    ) {
    ...
    cleanupActiveRenderers(
        ...
        Object.keys(this._activeRenderers)
            ...
            .filter(neid => {
                ...
                return (
                    this._currentNarrativeElement === undefined ||
    ...
    async getBackgroundAudioAssets(backgroundIds: string[]) {
        return Promise.all(
            backgroundIds.map(assetId =>
                this._fetchers.assetCollectionFetcher(assetId),
            ),
        )
    }
    ...
    async _handleBackgroundRendering(representation: Representation) {
        ...
        if (newBackgrounds.length === 0) {
            this.stopCurrentBackgroundRenderers(null)
            return
        }

        // gets all the background asset collections and picks the audio srcs
        const assetCollections = await this.getBackgroundAudioAssets(
            newBackgrounds,
        )
        ...
        assetCollections.forEach(ac => {
            if (ac.assets.audio_src) {
                ...
                if (!this.hasRendererForBackground(src)) {
                    ...
                    const newBackgroundRenderer = this.createNewBackgroundRenderer(
                        ac,
                        src,
                    )
    ...
    _createNewRenderer(
        ...
        if (newRenderer) {
            ...
            if (
                ...
                // eslint-disable-next-line max-len
                newRenderer.setChoiceToRepresentationWithLabel(
                    this._rendererState.lastSwitchableLabel,
                )
            }
        } else {
            logger.error(
                `Do not know how to render ${representation.representation_type}`,
            )
        }
    ...
    updateControlAvailability() {
        ...
        const hideList = getControlHideList(
            this._currentNarrativeElement,
            this._story,
        )
        ...
        if (this._currentRenderer) {
            ...
            if (!rend.inVariablePanel) {
                return this._controller
                    ...
                    .catch(err => {
                        logger.error(
                            "Could not get valid next steps to set next button availability",
                            err,
                        ) // eslint-disable-line max-len
    ...
    _getRenderer(
        ...
        Object.keys(this._activeRenderers).forEach(rendererNEId => {
            if (
                rendererNEId === narrativeElement.id &&
        ...
        if (!newRenderer) {
            ...
            if (newRenderer)
                this._activeRenderers[narrativeElement.id] = newRenderer
        }
    ...
    _rendererLookahead(narrativeElement: NarrativeElement): Promise<any> {
        ...
        return Promise.all([
            ...
            const renderPromises = allIds.map(neid => {
                ...
                if (neObj && neObj.body.representation_collection_target_id) {
                    return this._fetchers // eslint-disable-next-line max-len
                        .representationCollectionFetcher(
            ...
            this.updateControlAvailability()
            return Promise.all(renderPromises) // Clean up any renderers that are not needed any longer
                .then(() => {
    ...
    _runBackgroundLookahead() {
        ...
        Object.keys(this._activeRenderers).forEach(neid => {
            ...
            if (representation.asset_collections.background_ids) {
                // eslint-disable-next-line max-len
                representation.asset_collections.background_ids.forEach(
                    backgroundAssetCollectionId => {
                        if (!(backgroundAssetCollectionId in backgroundIds)) {
                            ...
                            if (
                                !this._previousNeId ||
                                (this._previousNeId &&
                                    this._previousNeId !== neid)
                            ) {
                                fwdBackgroundAcIds.push(
                                    backgroundAssetCollectionId,
                                )
                            }
                        }
        ...
        Promise.all(assetCollectionPromises)
            ...
            .then(backgroundSrcObjs => {
                ...
                backgroundSrcObjs.forEach(bgSrcObj => {
                    ...
                    if (fwdBackgroundAcIds.indexOf(ac.id) >= 0) {
                        // fwd
                        fwdRenderers[src] = this._upcomingBackgroundRenderers[
                            src
                        ]
                    }
                })
                ...
                Object.keys(this._upcomingBackgroundRenderers).forEach(src => {
                    if (
                        backgroundSrcObjs.filter(srcObj => srcObj.src === src)
                            .length === 0
                    ) {
     ...
     */
    _setBackgroundFades(fwdRenderers: Record<string, BaseRenderer>) {
                ...
                if (this._currentRenderer) {
                    ...
                    if (timeObj.remainingTime) {
                        if (timeObj.remainingTime < FADE_OUT_TIME) {
                            this._backgroundRenderers[id].fadeOut(
                                timeObj.remainingTime,
                            )
                        } else {
                            const fadeStartTime =
                                timeObj.currentTime +
    ...
    _initialise() {
        ...
        this._rendererState = {
            lastSwitchableLabel: "",
            // the label of the last selected switchable choice
