...
export const getSetting = settingName => {
    ...
    if (inSMPWrapper()) {
        ...
        settingValue = new URLSearchParams(window.location.search).get(
            settingName,
        )
    }
...
export const getVariableOverrides = () => {
    ...
    if (varNames.length !== varVals.length) {
        logger.info(
            `Query Parameter variable failed - number of name and value does not match`,
        )
        return []
    }
...
export const copySelection = (e: Record<string, any>) => {
    e.target.select()
...
export const scrollToTop = () => {
    window.setTimeout(() => {
        ...
        if (
            document.getElementsByClassName("taster-offsite-panel").length >
                0 &&
            document.getElementsByClassName("taster-offsite-panel")[0]
                .scrollIntoView
        ) {
            document
                .getElementsByClassName("taster-offsite-panel")[0]
                .scrollIntoView()
        } else if (
...
export const LOOPING_AUDIO_AC_TYPE =
...
export const preventEventDefault = (event: Event) => {
    ...
    if (event && event.target) {
        ...
        if (tgt.classList && !tgt.classList.contains(SLIDER_CLASS)) { 
 ...
 */
export const handleButtonTouchEvent = (
...
export const replaceEscapedVariables = (textContent, controller) => {
    ...
    return controller.getVariableState().then(vState => {
        ...
        const replacedText = textContent.replace(/\$\{(.*?)\}/g, (m, c) =>
            getVal(c),
        )
        return replacedText
    })
...
export const getControlHideList = (ne, story) => {
    ...
    if (ne && ne.meta.storyplayer && ne.meta.storyplayer.hide_controls) {
