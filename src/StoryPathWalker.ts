...
export default class StoryPathWalker extends EventEmitter {
    ...
    _getRepresentationCollections(
        path: Array<PathGather>,
    ): Promise<Array<PartialStoryPathItem>> {
        const promises = []
        path.forEach(pathGather => {
            // eslint-disable-next-line max-len
            const representationCollectionId =
                pathGather.ne.body.representation_collection_target_id

            if (representationCollectionId) {
                promises.push(
                    this._representationCollectionFetcher(
                        representationCollectionId,
                    ),
                )
            }
        })
    ...
    ): Promise<Array<StoryPathItem>> {
        const promises = []

        this._pathmap.forEach(pathItem => {
            promises.push(
                representationReasoner(pathItem.representation_collection),
            )
        })

        return Promise.all(promises).then(representations => {
            representations.forEach((repres, i) => {
                const fullPathItem = convertPartialToFull(
                    this._pathmap[i],
                    repres,
                )
    ...
    parseStory(storyId: string) {
        ...
        this._storyReasonerFactory(storyId).then(linearReasoner => {
            ...
            const _handleNarrativeElementChanged = (
                narrativeElement: NarrativeElement,
            ) => {
                const parentStories = [storyId].concat(
                    this._getStoryArray(linearReasoner, []),
                )
                const pathItem = {
                    stories: parentStories,
                    ne: narrativeElement,
                }
                const previousVisits = path.find(
                    pi => pi.ne.id === narrativeElement.id,
                )

                if (previousVisits !== undefined) {
                    logger.info(
                        "Linearity checker returned to previously visited node; story not linear",
