...
const _getLongListVariableSetter = (
    varName: string,
    variableDecl: Record<string, any>,
    getVariableValue: (...args: Array<any>) => any,
    setVariableValue: (...args: Array<any>) => any,
) => {
    ...
    getVariableValue(varName).then(varValue => {
...
const _getBooleanVariableSetter = (
    ...
    varInput.appendChild(varInputSelect)
    getVariableValue(varName).then(varValue => setSelected(varValue))
    return varInput
}
...
const _getNumberRangeVariableSetter = (
    ...
    behaviourVar: Record<string, any>,
    getVariableValue: (...args: Array<any>) => any,
    setVariableValue: (...args: Array<any>) => any,
) => {
    const varInput = document.createElement("div")
...
const getVariableSetter = (
    ...
    variableDiv.id = `romper-var-form-${behaviourVar.variable_name.replace(
        "_",
        "-",
    )}`
...
export const buildPanel = (
    ...
    const pauseWhileShowing =
        behaviour.pause_content !== undefined ? behaviour.pause_content : true
    ...
    const {
        ...
        okButton,
        statusSpan,
    } = createVarPanelElements(
        behaviour.panel_label,
        behaviour.background_colour,
    )
    player.disableControls()
    overlayImageElement.id = behaviour.id

    renderer._setBehaviourElementAttribute(
        overlayImageElement,
        "variable-panel",
    )

    const behaviourVariables = behaviour.variables
    getVariableState().then(storyVariables => {
        ...
        let statusText = `${currentQuestion + 1} of ${
            behaviourVariables.length
        }`
        ...
        const changeSlide = (fwd: boolean) => {
            ...
            statusText = `${currentQuestion + 1} of ${
                behaviourVariables.length
            }`
            statusSpan.textContent = statusText
            return false
        ...
        setTimeout(() => {
            overlayImageElement.classList.add("active")
        }, 200)
