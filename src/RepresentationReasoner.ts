...
export type RepresentationReasoner = (
        ...
        return evaluateConditions(
            ...
            .then(representationContainer => {
                if (representationContainer) {
                    return representationFetcher(
                        representationContainer[0].representation_id,
                    )
                }

                return Promise.reject(
                    new Error("no suitable representations found"),
                )
            })
            .then((rep: Representation) => {
                ...
                if (representation.choices) {
                    representation.choices.forEach(choice => {
                        promiseArray.push(
                            representationFetcher(
                                choice.choice_representation_id,
                            ),
                        )
                    })
            ...
            .then((reps: Array<Representation>) => {
                ...
                if (representation.choices) {
                    representation.choices = representation.choices.map(
                        choice => ({
                            label: choice.label,
                            choice_representation_id:
                                choice.choice_representation_id,
                            choice_representation:
                                repsId[choice.choice_representation_id],
                        }),
                    )
                }
