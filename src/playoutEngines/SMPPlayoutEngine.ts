...
import { SMPPlayListItem, SMPPlayList } from "../types/index"
...
class SMPPlayoutEngine extends BasePlayoutEngine {
    ...
    _playing: boolean
    _smpPlayerInterface: Record<string, any>
    _fakeItemRendererId: string | null | undefined
    _fakeItemDuration: number
    _fakeItemLoaded: boolean
    _fakeEventEmitter: Record<string, any>
    _volume: number
    _backgroundMix: number
    ...
    constructor(player: Player) {
        ...
        this._handleStartButtonClicked = this._handleStartButtonClicked.bind(
            this,
        )
        ...
        this._smpPlayerInterface.addEventListener(
            "volumechange",
    ...
    _createSecondaryPlayoutEngine(player: Player) {
        ...
        switch (playoutToUse) {
            case PLAYOUT_ENGINES.DOM_SWITCH_PLAYOUT:
                ...
                this._secondaryPlayoutEngine = new DOMSwitchPlayoutEngine(
                    player,
                )
                break
    ...
    setFbMix(fbMixValue) {
        ...
        this._player.emit(PlayerEvents.AUDIO_MIX_CHANGED, {
            id: AUDIO_MIX_EVENT_LABEL,
            label: AUDIO_MIX_EVENT_LABEL,
            value: fbMixValue,
        })
    }
    ...
    async queuePlayout(rendererId: string, mediaObj: Record<string, any>) {
        ...
        if (
            "loop" in this._media[rendererId].media &&
            this._media[rendererId].media.loop
        ) {
        ...
        if (url.indexOf("http") !== 0) {
            playlistItem = {...playlistItem, versionID: url, kind}
            ...
            playlistItem = {...playlistItem, href: url, kind}
        }
        ...
        if (isTrimmed && mediaObj.outTime > 0) {
            playlistItem = {...playlistItem, out: mediaObj.outTime}
        }

        const playlist: SMPPlayList = {
            summary: rendererId,
            options: {
                useCredentials: true,
            },
        ...
        playlist.options.useCredentials = isPid
            ? false
            : ["MPD", "InitializationSegment", "MediaSegment", "Player"]
    ...
    setPlayoutActive(rendererId: string) {
        ...
        if (this._permissionToPlay) {
            ...
            if (this._media[rendererId].loadedPlaylist) {
                ...
                this._smpPlayerInterface.loadPlaylistFromCollection(
                    rendererId,
                    true,
                )
            }
            ...
            if (!this._playing) {
                const pauseFunction = () => {
                    this._smpPlayerInterface.removeEventListener(
                        "playing",
                        pauseFunction,
                    )
    ...
    startNonAVPlayout(rendererId, duration = 0) {
        ...
        this._smpPlayerInterface.addEventListener(
            "mediaItemInfoChanged",
            this._smpFakeLoad,
        )
    }

    stopNonAVPlayout(rendererId: string | null | undefined) {
    ...
    off(
        ...
        if (rendererPlayoutObj.active) {
            ...
            const index = rendererPlayoutObj.queuedEvents.findIndex(
                qe => qe.event === event && qe.callback === callback,
            )
