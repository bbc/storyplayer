...
export default class DOMSwitchPlayoutEngine extends BasePlayoutEngine {
    ...
    constructor(player: Player) {
        ...
        const inactiveBufferingOverride = getSetting(
            OVERRIDE_INACTIVE_BUFFERING,
        )
    ...
    _shakaUpdateBandwidth(rendererId: string) {
        ...
        if (rendererPlayoutObj._shaka) {
            const newBandwidth = rendererPlayoutObj._shaka.getStats()
                .estimatedBandwidth
    ...
    _printActiveMSEBuffers() {
        if (this._activePlayer && this._activePlayer.mediaElement) {
            ...
            for (i = 0; i < bufferRanges; i += 1) {
                ...
                // eslint-disable-next-line no-console
                logger.debug(
                    `BUFFER: Buffer Range ${i}: ` + `${start} - ` + `${end}`,
                )
            ...
            if (validPlayback !== true) {
                // eslint-disable-next-line no-console
                logger.debug(
                    "BUFFER WARNING: current playback time outside of buffered range",
                )
            }
        ...
        if (!rendererPlayoutObj.mediaElement) {
            if (rendererPlayoutObj.media.type === MEDIA_TYPES.FOREGROUND_AV) {
                ...
                videoElement.className =
                    "romper-video-element romper-media-element-queued"
                videoElement.crossOrigin = "anonymous"
                rendererPlayoutObj.mediaElement = videoElement

                this._player.mediaTarget.appendChild(
                ...
                const videoElement = document.createElement("video")
                videoElement.className =
                    "romper-audio-element romper-media-element-queued"
                videoElement.crossOrigin = "anonymous"
                videoElement.setAttribute("tabindex", "-1")
                rendererPlayoutObj.mediaElement = videoElement
                ...
                this._player.backgroundTarget.appendChild(
                    rendererPlayoutObj.mediaElement,
                )
            }
        }
    ...
    unqueuePlayout(rendererId: string) {
        ...
        if (rendererPlayoutObj.mediaType) {
            switch (rendererPlayoutObj.mediaType) {
                ...
                default:
                    logger.error(
                        "Cannot handle this mediaType (unqueuePlayout)",
                    )
            }
        }
        ...
        if (
            ...
            this._player.mediaTarget.removeChild(
                rendererPlayoutObj.mediaElement,
            )
        } else {
            this._player.backgroundTarget.removeChild(
                rendererPlayoutObj.mediaElement,
            )
        }
        ...
        if (!rendererPlayoutObj.active) {
            if (rendererPlayoutObj.mediaType) {
                switch (rendererPlayoutObj.mediaType) {
                    ...
                    default:
                        logger.error(
                            "Cannot handle this mediaType (loadSource)",
                        )
                }
            }

            super.setPlayoutActive(rendererId)
            rendererPlayoutObj.mediaElement.classList.remove(
            ...
            if (
                this._playing &&
                rendererPlayoutObj.media &&
                rendererPlayoutObj.media.url
            ) {
                this.play()
            }
            ...
            if (rendererPlayoutObj.media && rendererPlayoutObj.media.type) {
                if (
                    rendererPlayoutObj.media.type ===
                        MEDIA_TYPES.FOREGROUND_AV ||
                    rendererPlayoutObj.media.type === MEDIA_TYPES.FOREGROUND_A
                ) {
        ...
        if (rendererPlayoutObj.active) {
            if (rendererPlayoutObj.mediaType) {
                switch (rendererPlayoutObj.mediaType) {
                    ...
                    default:
                        logger.error(
                            "Cannot handle this mediaType (setPlayoutInactive)",
                        )
                }
            }
            ...
            rendererPlayoutObj.mediaElement.classList.add(
                "romper-media-element-queued",
            )
            ...
            if (promise !== undefined) {
                promise
                    ...
                    .catch(error => {
                        logger.warn(
                            error,
                            "DOMSwitchPlayotEngine Not got permission to play",
            ...
            const timeoutId = setInterval(() => {
                mediaElement
                    ...
                    .catch(error => {
                        ...
                        logger.warn(
                            error,
                            " DOMSwitchPlayoutEngine set Timer Not got permission to play",
                        )
                        // Auto-play was prevented
                        clearInterval(timeoutId)
                    })
    ...
    _handleVolumeClicked(event: Record<string, any>): void {
    ...
    _toggleMute(event: Record<string, any>) {
        const rendererPlayoutObj = this._media[event.id]
    ...
    _cleanUpSubtitles(rendererId: string) {
        ...
        if (rendererPlayoutObj.mediaSubsTrack) {
            ...
            const videoTrackParent =
                rendererPlayoutObj.mediaSubsTrack.parentNode
    ...
    _showHideSubtitles(rendererId: string) {
        ...
        if (rendererPlayoutObj.media.subs_url && this._subtitlesShowing) {
            rendererPlayoutObj.mediaSubsTrack = (document.createElement(
                "track",
            ) as any) as HTMLTrackElement
            ...
            rendererPlayoutObj.mediaSubsTrack.src =
                rendererPlayoutObj.media.subs_url
