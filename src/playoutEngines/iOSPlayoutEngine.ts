...
export default class iOSPlayoutEngine extends BasePlayoutEngine {
    ...
    constructor(player: Player) {
        ...
        this._foregroundMediaElement.setAttribute(
            "disablePictureInPicture",
            "true",
        )

        this._backgroundMediaElement.setAttribute(
            "disablePictureInPicture",
            "true",
        )
    ...
    setPermissionToPlay(value: boolean) {
        ...
        super.setPermissionToPlay(value, false)
    }
    ...
    attachEverythingToActive(rendererId: string, wait = false) {
        ...
        if (mediaObj) {
            if (mediaObj.type) {
                ...
                if (
                    mediaObj.type === MEDIA_TYPES.FOREGROUND_AV ||
                ...
                if (
                    ...
                    rendererPlayoutObj.queuedEvents.length > 0
                ) {
    ...
    _setLoopAttribute(foreground: boolean, loop: boolean | null | undefined) {
                ...
                callback = e => {
                    if (e !== undefined) {
                        if (!e.target.duration) {
                            logger.info(
                                `Received ended event with no duration. ` +
