...
class SMPControls extends BaseControls {
    ...
    _uiUpdateQueueTimer: ReturnType<typeof setTimeout>
    _volumeControls: HTMLDivElement
    _masterValueLabel: HTMLSpanElement
    _chapterButton: HTMLButtonElement
    _scrubEnabled: boolean
    _scrubTimePoller: ReturnType<typeof setInterval>
            ...
            switch (e.controlId) {
                case "back_interval_button": {
                    ...
                    if (e.labels) {
                        ...
                        if (beforeTime !== undefined) {
                            seekBackFrom = beforeTime
                        }

                        if (afterTime !== undefined) {
                            seekBackTo = afterTime
                        }
                    }
                ...
                case "seek_bar": {
                    ...
                    if (e.labels) {
                        ...
                        if (before_seek_time !== undefined) {
                            scrubFrom = before_seek_time
                        }
                ...
                case "pause":
                    this._logUserInteraction(
                        AnalyticEvents.names.PLAY_PAUSE_BUTTON_CLICKED,
                        "play",
                        "pause",
                    )
                ...
                case "enter_picture_in_picture_mode":
                    this._logUserInteraction(
                        AnalyticEvents.names.PIP_MODE_CHANGED,
                        "normal",
                        "pip",
                    )
                ...
                case "previous_button":
                    this._logUserInteraction(
                        AnalyticEvents.names.BACK_BUTTON_CLICKED,
                    )
                ...
                case "next_button":
                    this._logUserInteraction(
                        AnalyticEvents.names.NEXT_BUTTON_CLICKED,
                    )
    ...
    _processUiUpdateQueue() {
        ...
        if (this._smpPlayerInterface.playlist === null) {
            this._uiUpdateQueueTimer = setTimeout(() => {
                this._processUiUpdateQueue()
            }, 1000)
            return
        }
        ...
        const controlsConfig = this._uiUpdateQueue.reduce(
            (combinedConfig, nextValue) => {
                return {...combinedConfig, ...nextValue}
            },
            {},
        )
    ...
    _createVolumeOverlay() {
        const smpVolumeBox = createElementWithClass(
            "div",
            "smp-volume-overlay",
    ...
    _createMixControl() {
        ...
        const fbMixSliderLabel = createElementWithClass(
            "div",
            "audio-mix-label",
            ["audio-slider-label"],
        )
        ...
        const fbMixSlider = createElementWithClass(
            "input",
            "audio-mix-slider",
            ["audio-slider"],
        )
    ...
    _setMixerLabel() {
        const slider = (document.getElementById("audio-mix-slider") as HTMLInputElement)
    ...
    _createMasterVolumeControl() {
        const masterContainer = createElementWithClass(
            "div",
            "audio-master-box",
            ["audio-volume-box"],
        )
        const masterSliderLabel = createElementWithClass(
            "div",
            "audio-master-label",
            ["audio-slider-label"],
        )
        ...
        const masterSlider = createElementWithClass(
            "input",
            "audio-master-slider",
            ["audio-slider"],
        )
        ...
        masterSlider.value = this._smpPlayerInterface.volume
            ? `${this._smpPlayerInterface.volume * 11}`
            : "5"
        masterSlider.step = 1
    ...
    _handleMuteClick(muted) {
        if (muted) {
            ...
            this._masterValueLabel.textContent = `${Math.round(
                this._smpPlayerInterface.volume * 11,
            )}`
        }
    }
    ...
    _createChapterButton() {
        ...
        const chapterButton = createElementWithClass(
            "button",
            "smp-chapter-button",
            [
                "p_button",
    ...
    connectScrubBar(renderer) {
        ...
        this._scrubTimePoller = setInterval(() => {
            ...
            this._playoutEngine.setNonAVPlayoutTime(
    ...
    enablePlayButton() {
        ...
        try {
            document
                .getElementById("p_audioui_playpause")
    ...
    setSubtitlesAboveElement(behaviourElement) {
        ...
        const playerHeight = this._playoutEngine._smpPlayerInterface.container
            .offsetHeight
