...
class Player extends EventEmitter {
    ...
    _volume: Overlay
    _representation: Overlay
    _icon: Overlay
    _linkChoice: Record<string, any>
    _analytics: AnalyticsLogger
    _assetUrls: AssetUrls
    _volumeEventTimeouts: Record<string, any>
    _userInteractionStarted: boolean
    _numChoices: number
    _choiceIconSet: Record<string, Promise<Record<string, any>>>
    _visibleChoices: Record<number, HTMLElement>
    _choiceCountdownInterval: ReturnType<typeof setTimeout> | null | undefined
    _countdowner: HTMLDivElement
    _countdownContainer: HTMLDivElement
    _aspectRatio: number
    _dogImage: HTMLDivElement
    _loadingLayer: HTMLElement
    _privacyDiv: HTMLDivElement | null | undefined
    _currentRenderer: BaseRenderer | null | undefined
    ...
    constructor(
        ...
        this._removeExperienceOverlays = this._removeExperienceOverlays.bind(
            this,
        )
        ...
        this._spatialNavigationHandler = new SpatialNavigationHandler(
            true,
            true,
        )
        ...
        this._representation = this._createOverlay(
            "representation",
    ...
    _createPlayoutEngine() {
        ...
        if (isDebug()) {
            ...
            this.playoutEngine = proxyWrapper(
                "PlayoutEngine",
                this.playoutEngine,
            )
        }
    ...
    _createLayerElements() {
        ...
        this._player = createElementWithClass("div", "storyplayer", [
            "romper-player",
        ])
        // create the background layer
        this._backgroundLayer = createElementWithClass(
            "div",
            "storyplayer-background",
            ["romper-background"],
        )
        // create the foreground media layer
        this._mediaLayer = createElementWithClass("div", "media-layer", [
            "romper-media",
        ])

        if (!inSMPWrapper()) {
            this._loadingLayer = createElementWithClass(
                "div",
                "loading-layer",
                ["romper-loading"],
            )
            const loadingLayerInner = createElementWithClass(
                "div",
                "loading-layer-inner",
        ...
        this._guiLayer = createElementWithClass("div", "gui-layer", [
            "romper-gui",
        ])
        ...
        this._overlaysElement = createElementWithClass(
            "div",
            "romper-overlays",
            ["romper-overlays", "buttons-hidden", "romper-inactive"],
        )
     ...
     */
    _createModalLayer() {
        this._continueModalLayer = createElementWithClass(
            "div",
            "continue-modal",
            ["continue-modal"],
        )
        this._continueModalContent = createElementWithClass(
            "div",
    ...
    _buildSMPControls() {
        ...
        const ctrls = this._controls.getControls();
        if (ctrls) this._guiLayer.appendChild(ctrls)
    }
    ...
    _buildStandardControls() {
        ...
        if (!this.useExternalTransport()) {
            ...
            const activator = this._controls.getActivator();
            if (activator) this._guiLayer.appendChild(activator)
        }

        this._player.addEventListener(
            "touchend",
            this._handleTouchEndEvent.bind(this),
        )
    }
    ...
    _createOverlay(name: string, logFunction: (...args: Array<any>) => any) {
        ...
        overlay.on(OVERLAY_ACTIVATED_EVENT, clickedName => {
            this._overlays
                .filter(o => o.getName() !== clickedName)
    ...
    _keyPressHandler(event: KeyboardEvent) {
        ...
        if (event.code === "ArrowRight")
            this._spatialNavigationHandler.goRight()
    ...
    _setupButtonHandling() {
        ...
        this._controls.on(ButtonEvents.SUBTITLES_BUTTON_CLICKED, () =>
            this.emit(PlayerEvents.SUBTITLES_BUTTON_CLICKED),
        )

        this._controls.on(ButtonEvents.FULLSCREEN_BUTTON_CLICKED, () =>
            this._toggleFullScreen(),
        )

        this._controls.on(ButtonEvents.PLAY_PAUSE_BUTTON_CLICKED, e =>
            this.emit(PlayerEvents.PLAY_PAUSE_BUTTON_CLICKED, e),
        )

        this._controls.on(ButtonEvents.SEEK_FORWARD_BUTTON_CLICKED, () =>
            this.emit(PlayerEvents.SEEK_FORWARD_BUTTON_CLICKED),
        )

        this._controls.on(ButtonEvents.SEEK_BACKWARD_BUTTON_CLICKED, () =>
    ...
    addDog(src: string, position: Record<string, any>) {
    ...
    _setDogPosition(position: Record<string, any>) {
        const {top, left, width, height} = position
        const guiAspect =
            this._guiLayer.clientWidth / this._guiLayer.clientHeight
        ...
        if (leftGreaterThanRight(guiAspect, this._aspectRatio)) {
            ...
            this._dogImage.style.left = `${
                sideGap + (left / 100) * mediaWidth
            }px`
    ...
    _addContinueModal() {
        ...
        const restartButtonHandler = () => {
            ...
            this._logUserInteraction(
                AnalyticEvents.names.BEHAVIOUR_CANCEL_BUTTON_CLICKED,
            )
        ...
        const resumeExperienceButtonHandler = () => {
            ...
            this._logUserInteraction(
                AnalyticEvents.names.BEHAVIOUR_CONTINUE_BUTTON_CLICKED,
            )
    ...
    _handleTouchEndEvent(event: Record<string, any>) {
        ...
        if (!this._controls.getShowing()) {
            ...
            const openTriggerElements = [
                this._overlaysElement, // this._narrativeElementTransport, // TODO why?
            ...
            const closeTriggerElements = [
                this._overlaysElement, // this._narrativeElementTransport, // TODO why close on this?
            ]
            // Prevent touch being converted to click on button bar
            // (which would then trigger activate area mouseenter events)
            const proventClickTriggerElements = [this._controls.getControls()]
            ...
            if (closeTriggerElements.some(el => el === endTarget)) {
                ...
                proventClickTriggerElements.some(el => el === endTarget)
            ) {
                event.preventDefault()
            }
        }
     ...
     */
    showErrorLayer(message = 'Unknown Error', showControls = false) {
        if (showControls) {
    ...
    _createStartExperienceButton(options: Record<string, any>) {
        ...
        this._startExperienceButton.setAttribute(
            "title",
            "Play and accept terms",
        )

        this._startExperienceButton.setAttribute("aria-label", "Start")

        this._startExperienceButton.setAttribute(
            "spatial-navigation-object",
            "content",
        )
     ...
     */
    _createStartImage(options: Record<string, any>) {
        if (!this._startExperienceImage) {
            ...
            this._startExperienceImage.setAttribute(
                "alt",
     ...
     */
    setupExperienceOverlays(options: Record<string, any>) {
    ...
    _createPrivacyNotice(options: Record<string, any>) {
    ...
    _createSharedOverlays(options: Record<string, any>) {
    ...
    _createResumeOverlays(options: Record<string, any>) {
        this._createSharedOverlays(options)

        this._addContinueModal()
    }

    _createStartOverlays(options: Record<string, any>) {
        this._createSharedOverlays(options)
    ...
    _removeExperienceOverlays() {
        try {
            if (this._startExperienceButton) {
                this._guiLayer.removeChild(this._startExperienceButton) // this._startExperienceButton = null;
            }
            ...
            if (this._startExperienceImage) {
                this._mediaLayer.removeChild(this._startExperienceImage) // this._startExperienceImage = null;
            }
    ...
    _enableUserInteraction() {
        ...
        this.playoutEngine.setPermissionToPlay(true, startNow)
    ...
    _handleOverlayClick(event: Event) {
        if (this._controls.getShowing()) {
    ...
    setVolumeControlLevel(label: string, value: number) {
        ...
        if (volumeControl) {
            if (
                volumeControl.childNodes[1] &&
                volumeControl.childNodes[1].childNodes[2]
            ) {
                // set slider value
                const slider = (volumeControl.childNodes[1].childNodes[1] as HTMLInputElement)
                slider.value = `${value}`
                // and feedback div
                volumeControl.childNodes[1].childNodes[2].textContent = `${Math.floor(
                    10 * value,
                )}`
            }

            this.emit(PlayerEvents.VOLUME_CHANGED, {
                id,
                value,
                label,
            })
    ...
    setMuted(label: string, muted: boolean) {
        ...
        if (volumeControl) {
            ...
            this.emit(PlayerEvents.VOLUME_MUTE_TOGGLE, {
                id,
                label,
                muted,
            })
    ...
    _setMuteCallBack(id: string, label: string, muteButton: HTMLButtonElement) {
        return () => {
            ...
            this._logUserInteraction(
                AnalyticEvents.names.VOLUME_MUTE_TOGGLED,
                `${label}: ${!muted}`,
                `${label}: ${muted}`,
            )
        }
    ...
    _setVolumeCallback(
        ...
        return (event: Record<string, any>) => {
            ...
            if (value === 0) {
                ...
                const isMuted = muteButton.classList.contains(
                    "romper-muted-button",
                )
            ...
            this._volumeEventTimeouts[label] = setTimeout(() => {
                this._logUserInteraction(
                    AnalyticEvents.names.VOLUME_CHANGED,
                    null,
    ...
    addVolumeControl(id: string, label: string) {
        ...
        volumeControl.classList.add(
            `romper-volume-label-${label.toLowerCase()}`,
        )
        volumeControl.setAttribute(
            "aria-label",
            `Volume controls for ${label} audio`,
        )
        ...
        volumeRange.oninput = this._setVolumeCallback(
            id,
            label,
            levelSpan,
            muteButton,
        ).bind(this)
        volumeRange.onchange = this._setVolumeCallback(
            id,
            label,
            levelSpan,
            muteButton,
        ...
        muteButton.addEventListener(
            "touchend",
            handleButtonTouchEvent(
                this._setMuteCallBack(id, label, muteButton).bind(this),
            ),
        )
        muteButton.onclick = this._setMuteCallBack(id, label, muteButton).bind(
            this,
        )
    ...
    removeVolumeControl(id: string) {
        ...
        if (this._volume.getCount() === 0)
            this._controls.disableBackgroundAudio()
    }
    ...
    addRepresentationControl(id: string, src: string, label: string) {
        ...
        representationControl.classList.add(
            `romper-representation-choice-${id}`,
        )
        ...
        const representationIconClick = () => {
            this.emit(PlayerEvents.REPRESENTATION_CLICKED, {
                id,
            })
            ...
            this._logUserInteraction(
                AnalyticEvents.names.SWITCH_VIEW_BUTTON_CLICKED,
                null,
                id,
            )
        }
    ...
    activateRepresentationControl(id: string) {
        this._representation.removeClassFromElement(
            id,
    ...
    createBehaviourOverlay(behaviour: Record<string, any>): Overlay {
        const behaviourOverlay = this._createOverlay(
            "link-choice",
            this._logUserInteraction,
        )
    ...
    addTextLinkIconChoice(
        ...
        // eslint-disable-line max-len
        return this._addLinkChoiceContainer(
            behaviourElement,
            id,
            label,
            text,
    ...
    _addLinkChoiceContainer(
        ...
        const containerPromise = new Promise(resolve => {
            ...
            linkChoiceControl.setAttribute(
                "aria-label",
                `${this._numChoices}: ${label}`,
            )
            linkChoiceControl.setAttribute("data-link-choice", "inactive")
            const iconContainer = document.createElement("div")

            const choiceClick = () => {
                this.emit(PlayerEvents.LINK_CHOSEN, {
                    id,
                    behaviourId: behaviourElement.id,
                })
            ...
            if (text && src) {
                const linkChoiceIconSrc =
                    src !== "" ? src : this._assetUrls.noAssetIconUrl
    ...
    hideChoiceIcons() {
        const ctrls = this._controls.getControls();
        if (ctrls) ctrls.classList.add("icons-showing") // TODO - fix?
    ...
    showChoiceIcons(
        activeLinkId: string | null | undefined,
        overlayClass: string | null | undefined,
        behaviourOverlay: Record<string, any>,
        choiceCount: number,
    ) {
        // eslint-disable-line max-len
        this._hideRomperButtons()
        ...
        behaviourElement.setAttribute(
            "aria-label",
            "choice: enter number to choose",
        )
        const promisesArray = []
    ...
    addIconControl(
        ...
        const iconClick = () => {
            this.emit(PlayerEvents.ICON_CLICKED, {
                id,
            })
            ...
            this._logUserInteraction(
                AnalyticEvents.names.CHANGE_CHAPTER_BUTTON_CLICKED,
                null,
                id,
            )
        }
        ...
        icon.addEventListener("touchend", handleButtonTouchEvent(iconClick))
    ...
    setIconControl(id: string, src: string, selected = false) {
        ...
        if (iconControl) {
            ...
            const icon = (iconControl.children[0] as HTMLImageElement)
    ...
    enterCompleteBehavourPhase() {
        this._logRendererAction(
            AnalyticEvents.names.COMPLETE_BEHAVIOUR_PHASE_STARTED,
        )
    ...
    clearLinkChoices() {
        ...
        const ctrls = this._controls.getControls();
        if (ctrls) ctrls.classList.remove("icons-showing") // TODO - fix?
    ...
    // eslint-disable-next-line class-methods-use-this
    getLinkChoiceElement(full?: boolean | null | undefined): NodeListOf<Element> | [Element] {
        const linkChoices = document.querySelectorAll(
            '[data-behaviour="link-choice"]',
        )
    ...
    static _isFullScreen() {
        ...
        if (
            document.getElementsByClassName("ios-target-fullscreen").length > 0
        ) {
            isFullScreen = true
        }
    ...
    _setIosFullScreen(entering: boolean) {
        ...
        if (entering) {
            ...
            if (fullscreenContainer && fullscreenContainer.parentNode) {
                ...
                const parent = (fullscreenContainer.parentNode as HTMLElement)
                parent.style.height = "100vh"
                parent.style.width = "100vw"
            }

            this._playerParent.addEventListener(
                "touchmove",
                preventEventDefault,
            )
            ...
            this._playerParent.removeEventListener(
                "touchmove",
                preventEventDefault,
            )
    ...
    _enterFullScreen() {
        ...
        this.emit(DOM_EVENTS.TOGGLE_FULLSCREEN, {
            isFullScreen: true,
        })
    }
    ...
    _handleFullScreenEvent() {
        if (Player._isFullScreen()) {
            const smp =
                MediaFormats.getPlayoutEngine() === PLAYOUT_ENGINES.SMP_PLAYOUT
            ...
            if (leftGreaterThanRight(windowAspect, this._aspectRatio)) {
                // too wide
                ...
                this._player.style.marginLeft = `${
                    (100 - scaleFactor * 100) / 2
                }vw`

                // we need to center vertically manually for SMP
                if (smp) {
                    this._player.style.marginTop = `calc(50vh - ${
                        height / 2
                    }vw)`
    ...
    addFacebookWebviewOverride(target: HTMLElement) {
        const facebookiOSWebview =
            BrowserUserAgent.facebookWebview() && BrowserUserAgent.iOS()
        ...
        if (facebookiOSWebview && !overrideFacebookBlock) {
            ...
            fbWebviewDiv.innerHTML =
                '<div class="webview-error-div">' +
