...
export const REASONER_ERRORS: Record<string, string> = [
    "NO_BEGINNING",
    "ALREADY_ENDED",
    "CURRENTLY_RESOLVING",
    "NOT_STARTED",
...
export default class StoryReasoner extends EventEmitter {
    ...
    _storyEnded: boolean
    _resolving: boolean
    _parent: StoryReasoner | null
    _analytics: AnalyticsLogger
    _narrativeElements: Record<any, any>
    _subStoryReasoner: StoryReasoner | null
    _currentNarrativeElement: NarrativeElement | null
     ...
     * @return {void}
     */
    start(initialState: Record<string, any> = {}) {
        if (this._storyStarted) {
            logger.warn(
                "Calling reasoner start on story that has already started",
    ...
    _fetchVariablesFromStory() {
        if (this._story.variables) {
            ...
            return Promise.all(
                Object.keys(variableTree).map(storyVariableName => {
                    const storyVariableValue =
                        variableTree[storyVariableName].default_value
                    return this.getVariableValue(storyVariableName).then(
                        value => {
                            ...
                            logger.info(
                                `Variable ${storyVariableName} already has value ${value}`,
                            )
                            return null
                        },
                    )
    ...
    _applyInitialState(initialState: Record<string, any>) {
        ...
        const internalVarSetter = new InternalVariables(
            this._dataResolver,
            this._story.meta,
        )
    ...
    next() {
        ...
        if (this._storyEnded) {
            throw new ReasonerError(
                REASONER_ERRORS.ALREADY_ENDED,
                ERROR_MESSAGES.ALREADY_ENDED,
            )
        }
    ...
    chooseBeginning() {
        ...
        evaluateConditions(this._story.beginnings, this._dataResolver).then(
            startElement => {
                this._resolving = false

                if (startElement) {
                    this._setCurrentNarrativeElement(
                        startElement[0].narrative_element_id,
                    )
                } else {
                    this.emit(
                        ERROR_EVENTS, // eslint-disable-next-line max-len
                        new ReasonerError(
                            REASONER_ERRORS.NO_BEGINNING,
                            ERROR_MESSAGES.NO_BEGINNING,
                        ),
                    )
                }
            },
    ...
    getBeginning(story: Story): Promise<string | null | undefined> {
        return evaluateConditions(story.beginnings, this._dataResolver).then(
            beginnings => {
    ...
    _chooseNextNode() {
        ...
        evaluateConditions(
            this._currentNarrativeElement.links,
            this._dataResolver,
            ...
            if (nextElementChoices) {
                ...
                const overrideLink = nextElementChoices.find(
                    link => link.override_as_chosen,
                )
                ...
                this.emit(
                    ERROR_EVENTS, // eslint-disable-next-line max-len
                    new ReasonerError(
                        REASONER_ERRORS.NO_VALID_LINKS,
                        ERROR_MESSAGES.NO_VALID_LINKS,
                    ),
                )
            }
        })
        ...
        if (!(narrativeElementId in this._narrativeElements)) {
            this.emit(
                ...
                new ReasonerError(
                    ...
                    ERROR_MESSAGES.INVALID_LINK_TARGET,
                ),
            )
        } else {
            this._currentNarrativeElement = this._narrativeElements[
                narrativeElementId
            ]
            ...
            if (this._currentNarrativeElement.body.type === "STORY_ELEMENT") {
                ...
                if (this._currentNarrativeElement.body.story_target_id) {
                    const storyId = this._currentNarrativeElement.body
                        .story_target_id
                    ...
                    this._reasonerFactory(storyId, this._analytics)
                        .then(subStoryReasoner =>
                            this._initSubStoryReasoner(subStoryReasoner),
                        )
                        .catch(err => {
    ...
    setVariableValue(name: string, value: any) {
        this.emit(VARIABLE_EVENTS.VARIABLE_CHANGED, {
            name,
            value,
        })
        logger.info(
            `Setting variable in story reasoner '${name}' to ${JSON.stringify(
                value,
            )}`,
        )
    ...
    appendToHistory(narrativeElementId: string) {
        ...
        this._dataResolver
            .get(InternalVariableNames.PATH_HISTORY)
            .then(value => {
                let neList = []
    ...
    _addSubReasonerListeners(subStoryReasoner: StoryReasoner) {
        ...
        const branchBeginningCallback = () =>
            this.emit(REASONER_EVENTS.CHOICE_OF_BEGINNINGS)

        const branchLinkCallback = () =>
            this.emit(REASONER_EVENTS.CHOICE_OF_LINKS)
        ...
        subStoryReasoner.on(
            REASONER_EVENTS.CHOICE_OF_BEGINNINGS,
            branchBeginningCallback,
        )
    ...
    getParent(): StoryReasoner | null | undefined {
        return this._parent
    }
    ...
    _isFollowedByAnotherNode(reasoner: StoryReasoner): boolean {
        ...
        if (
            reasoner._currentNarrativeElement.links[0].link_type ===
            "NARRATIVE_ELEMENT"
        ) {
            return true
        }
     ...
     * to this one.
     */
    findPreviousNodeId(): string | null | undefined {
