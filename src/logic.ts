...
export default function evaluateConditions<T>(
    ...
    candidates.forEach(candidate =>
        JsonLogic.uses_data(candidate.condition).forEach(cv =>
            interestingVars.push(cv),
        ),
    )
    return Promise.all(
        interestingVars.map(interestingVar =>
            dataResolver
                .get(interestingVar)
                .catch(() => null)
                .then(value => ({
        ...
        .then(resolvedVars => {
            const evaluatedCandidates = candidates
                .map((candidate, i) => ({
                    i,
                    result: JsonLogic.apply(candidate.condition, resolvedVars),
                }))
            ...
            if (evaluatedCandidates.length > 0) {
                if (evaluatedCandidates.length > 1) {
                    logger.info(
                        `LOGIC: choice of ${evaluateConditions.length} conditions`,
                    )
                }
