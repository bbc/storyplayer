...
import AnalyticEvents, { AnalyticsLogger } from "./AnalyticEvents"
import Controller from "./Controller"

interface SegmentSummaryData {
    timing: Record<any, any>
    eventcounts: Array<any>
    chapter?: string
}
...
export default class AnalyticsHandler {
    ...
    userid: string
    _segmentSummaryData: SegmentSummaryData 
    ...
    _enhanceAnalytics(logData) {
        ...
        if (neId === undefined) {
            neId = this._controller.getCurrentNarrativeElement()
                ? this._controller.getCurrentNarrativeElement().id
                : "null"
        }
        ...
        if (
            renderer &&
            this._segmentSummaryData.timing.defaultDuration === null
        ) {
    ...
    _sumpausedTime() {
        ...
        const totalPausedTime =
            this._segmentSummaryData.timing.pausedTime + pausedTime
    ...
    _sumHiddenTime() {
        ...
        const totalHiddenTime =
            this._segmentSummaryData.timing.hiddenTime + hiddenTime
    ...
    _handleSegmentSummaries(appendedData) {
        ...
        if (
            appendedData.name ===
                AnalyticEvents.names.BROWSER_VISIBILITY_CHANGE &&
            ...
            appendedData.name === AnalyticEvents.names.BROWSER_VISIBILITY_CHANGE
        ) {
        ...
        if (
            appendedData.name ===
                AnalyticEvents.names.NARRATIVE_ELEMENT_CHANGE ||
    ...
    _saveSummaryData(appendedData) {
        const {
            ...
            pausedTime,
            hiddenTime,
        } = this._segmentSummaryData.timing
        const duration = Date.now() - startTime
        this._segmentSummaryData.timing.duration = duration
        this._segmentSummaryData.timing.playingTime =
            duration - pausedTime - hiddenTime
